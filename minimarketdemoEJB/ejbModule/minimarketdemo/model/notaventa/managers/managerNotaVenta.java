...
public class managerNotaVenta {
	...
	public void IngresarNotaDeVenta(RecRecepcionCabecera cabecera, double total) throws Exception {
		ProNotaVenta NuevaNotaVenta=new ProNotaVenta();
		NuevaNotaVenta.setProNotTotal(BigDecimal.valueOf(total));
		NuevaNotaVenta.setRecRecepcionCabecera(cabecera);
		NuevaNotaVenta.setProNotFecha(new Date());
		mDao.insertar(NuevaNotaVenta);
		cabecera.setRecCabEstado(false);
		mDao.actualizar(cabecera);
	}
	public List<ProNotaVenta> findAllNotasDeVenta() {
		return mDao.findAll(ProNotaVenta.class);
	}
}